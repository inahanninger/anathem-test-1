...
const TableOfContents = ({
  ...
  return <div className="mb-4 border rounded-md bg-white shadow-sm">
        ...
        <CollapsibleContent>
          <ScrollArea className="max-h-[calc(100vh-200px)] overflow-auto">
            <div className="px-2 py-1">
              {items.map(item => <Button key={item.id} variant="ghost" size="sm" className={`w-full justify-start mb-0.5 text-left pl-${item.level * 2} text-xs py-1 h-auto`} onClick={() => onSelectItem(item.id)}>
                  {item.title}
                </Button>)}
            </div>
          </ScrollArea>
