  ...
  if (alwaysEditable) {
    return <div className="w-full">
        <div className="space-y-2">
          {fieldType === "textarea" ? <Textarea ref={textareaRef} value={value} onChange={e => setValue(e.target.value)} placeholder={placeholder} rows={1} className="focus:ring-1 focus:ring-blue-400 border-gray-200 resize-none text-s h-auto min-h-[2.5rem] overflow-visible" /> : <Input value={value} onChange={e => setValue(e.target.value)} placeholder={placeholder} className="focus:ring-1 focus:ring-blue-400 border-gray-200 text-xs" />}
        </div>
      </div>;
  }
  return <div className="w-full">
      {isEditing ? <div className="space-y-2">
          {fieldType === "textarea" ? <Textarea ref={textareaRef} value={value} onChange={e => setValue(e.target.value)} placeholder={placeholder} className="focus:ring-1 focus:ring-blue-400 border-gray-200 resize-none text-xs h-auto min-h-[2.5rem] overflow-visible" autoFocus /> : <Input value={value} onChange={e => setValue(e.target.value)} placeholder={placeholder} className="focus:ring-1 focus:ring-blue-400 border-gray-200 text-xs" autoFocus />}
